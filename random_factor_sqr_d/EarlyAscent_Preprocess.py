...
if 1: # run exp
    for d in d_list:
        ...
        if d == 1:
            ...
            # generate centers
            M = 3
            print('generate m_centers uniformly at random for d>=2')
            m_centers = generate_M_uniform_unit_vectors(M, d)
            print('generate w_centers uniformly at random for d>=2')
            w_centers = generate_M_uniform_unit_vectors(M, d)
        
        print('generate pis')
        pis = generate_mixure_coefficients(M, q)
    
        variance = 2.0
        s = variance*0.5* np.sqrt(d) # sigma
        t = variance* np.sqrt(d) # tau
        sm = 0.05* np.sqrt(d) # sigma_mu
        sw = 0.05* np.sqrt(d) # sigma_w
        ...
        if d == 1:
            ...
            new_m_center = generate_M_uniform_unit_vectors(1, d).reshape(-1)
            print('generate new_w_center')
            new_w_center = generate_M_uniform_unit_vectors(1, d).reshape(-1)
