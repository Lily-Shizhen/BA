...
if 1:  # define parameters
    S = 64
    num_k = 128  #length of examples
    k_list = [1, 2, 4, 8, 16, 32, 64, 128] #indices of samples
    k_list = [a - 1 for a in k_list]
    k_array = np.array(k_list)
    k_list = [r'${}$'.format(a) for a in k_list]
    K = 5000 #number of tasks
    z = 1  # 1× standard deviation
    linewidth = 3.0
...
if 1:  # preprocess
    for index, (sm, sw) in enumerate(zip(sm_list, sw_list)):
        ...
        # Change here: pass d=200 (or whatever dimension you want).
        ...
        if 1:
            ...
            if 1:
                ...
                # Just load existing files if they exist
