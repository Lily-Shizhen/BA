...
def train_model(args, prior, model, optimizer, epoch):
    ...
    for i in tqdm(range(args.steps)):
        ...
        input_ = torch.from_numpy(input_).to(device).float()
    ...
    for col, d in enumerate(d_list):
        ...
        for epoch in range(1, args.epochs+1):
            ...
            if epoch == 1:
                ...
                axes[epoch-1,col].set_ylim([1/10**2,10**1])
                axes[epoch-1,col].set_yscale('log')
            
                axes[epoch-1,col].set_title(r'$d=$'+' '+str(d))
                
            if col==0:
                axes[epoch-1,col].text(-12.0, 0.1, f"EP={epoch}", fontsize=48, verticalalignment='center', horizontalalignment='right', rotation='vertical')
    handles, labels = axes[0,2].get_legend_handles_labels()
    fig.legend(handles, labels, bbox_to_anchor=(0.5, 1.05), fontsize=48, ncol=2, loc='upper center')
    fig.text(0.5, -0.02, 'Number of In-Context Examples '+r'$(k)$', ha='center', va='center', fontsize=48)
    #plt.tight_layout()
